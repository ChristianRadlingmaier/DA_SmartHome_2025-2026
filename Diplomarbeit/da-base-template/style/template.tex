...
% Avoid TeX main-memory overflow when many PDF pages are attached.
% pdfpages repeatedly appends to \@filelist; we disable that only during include.
\makeatletter
\newcommand{\includepdfnofilelist}[2][]{%
  \begingroup
...
\includepdfnofilelist[
    clip=0mm 0mm 0mm 0mm,
    pages=$if(da-kiquellen.seiten)$$da-kiquellen.seiten$$else$-$endif$,
    frame,
    scale=.73,
